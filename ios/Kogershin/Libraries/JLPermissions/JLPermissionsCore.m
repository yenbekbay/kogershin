...
- (NSString *)defaultTitle:(NSString *)authorizationType {
    return [NSString stringWithFormat:NSLocalizedString(@"\"%@\" Would Like to Access Your %@", nil), [self appName], authorizationType];
}
...
- (NSString *)defaultCancelTitle {
    return NSLocalizedString(@"Don't Allow", nil);
}
...
- (void)displayReenableAlert {
    NSString *message = [NSString stringWithFormat:NSLocalizedString(@"Please go to Settings -> Privacy -> %@ to re-enable %@'s access.", nil), [self permissionName], [self appName]];
...
- (NSError *)systemDeniedError:(NSError *)error {
    return [NSError errorWithDomain:@"SystemDenied" code:JLPermissionSystemDenied userInfo:[error userInfo]];
}
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if (canceled) {
            NSError *error = [NSError errorWithDomain:@"UserDenied" code:JLPermissionUserDenied userInfo:nil];
